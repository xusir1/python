...
class Person:
注意，类的名称一般用大写字母开头，这是惯例。

接下来，一般都要编写构造函数，在写这个函数之前，先解释一下什么是构造函数。

class Person:
def init(self, name, lang, website):
self.name = name
self.lang = lang
...
在这里要明确一个基本概念，类就是一种对象类型，和跟前面学习过的数值、字符串、列表等等类型一样。比如这里构建的类名字叫做Person，那么就是我们要试图建立一种对象类型，这种类型被称之为Person，就如同有一种对象类型是list一样。
...
注意观察：info=Person("hiekay","python","hiekay.github.io")，这句话就是将类Person实例化了。也就是在内存中创建了一个对象，这个对象的类型是Person类型，这个Person类型是什么样子的呢？就是init()所构造的那样。在实例化时，必须通过参数传入具体的数据：name="hiekay",lang="python",website="hiekay.github.io"。这样在内存中就存在了一个对象，这个对象的类型是Person，然后通过赋值语句，与变量info建立引用关系。请看官回忆以前已经讲述过的变量和对象的引用关系。

类、实例，这两个概念会一直伴随着后续的学习，并且，在很多的OOP模型中，都会遇到这两个概念。为了让看官不晕乎，这里将它们进行比较（注意：比较的内容，参考了《Learning Python》一书）
...
self的作用
在构造函数中，第一个参数是self，但是在实例化的时候，似乎没有这个参数什么事儿，那么self是干什么的呢？

self是一个很神奇的参数。

在Person实例化的过程中，数据"hiekay","python","hiekay.github.io"通过构造函数（init()）的参数已经存入到内存中，并且这些数据以Person类型的面貌存在组成一个对象，这个对象和变量info建立的引用关系。这个过程也可说成这些数据附加到一个实例上。这样就能够以:object.attribute的形式，在程序中任何地方调用某个数据，例如上面的程序中以info.name得到"hiekay"这个数据。这种调用方式，在类和实例中经常使用，点号“.”后面的称之为类或者实例的属性。

这是在程序中，并且是在类的外面。如果在类的里面，想在某个地方使用传入的数据，怎么办？

在类内部，我们会写很多不同功能的函数，这些函数在类里面有另外一个名称，曰：方法。那么，通过类的构造函数中的参数传入的这些数据也想在各个方法中被使用，就需要在类中长久保存并能随时调用这些数据。为了解决这个问题，在类中，所有传入的数据都赋给一个变量，通常这个变量的名字是self。注意，这是习惯，而且是共识，所以，看官不要另外取别的名字了。
...
class Person:
    def __init__(self, name, lang, website):
        ...
        self.lang = lang
        self.website = website
        self.email = "hiekay@gmail.com"     #这个属性不是通过参数传入的
...
info.lang= python
info.website= hiekay.github.io
info.email= hiekay@gmail.com    #打印结果
...
构造函数的参数
前面已经说过了，构造函数init就是一个函数，只不过长相有点古怪罢了。那么，函数中的操作在构造函数中依然可行。比如：

def init(self,args):
pass
这种类型的参数：args和前面讲述函数参数一样。但是，self这个参数是必须的，因为它要来建立实例对象。
...
hiekay = Person("hiekay")     #导入一个数据name="hiekay"，其它默认值
info = Person("hiekay",lang="python",website="hiekay.github.io")    #全部重新导入数据
...
info.website= hiekay.github.io
在这段代码中，看官首先要体会一下，“类是实例的工厂”这句话的含义，通过类Person生成了两个实例：hiekay、info
